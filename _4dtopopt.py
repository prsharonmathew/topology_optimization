...
from particle_bin_plot import ParticleGridPlotter
from particle_actuation import ParticleActuationAnalyzer
...
real = ti.f64   # use double , 64 bit floating point precision
USE_GPU = True
#if set to True, use GPU for computation
if USE_GPU:
...
dx = L / n_grid  # grid spacing, 0.01m
inv_dx = 1 / dx 
particle_density = 2 
#particles per grid cell per dimension. 0,5*dx = 0,005m. There will be 2^2 = 4
#particles per grid cell in 2D, and 2^3 = 8 particles per grid cell in 3D.
...
block_size = 0.1  # side length of design domain, m. for 3D.
block_offset = (0.1, H, 0.5-block_size/2)  # for walker
#block_offset = (0.5-block_size/2, H, 0.5-block_size/2)  # for rotator
#The cube is placed above the ground, 0,1m apart from x-axis, and centered at
#z=0.5m.

max_steps = 5000  
#total simulation time = max_steps * dt, s. 0,5s here. However in the article it
#is mentioned as 1s.
steps = max_steps
...
# Filtering
filter_radius = dx / particle_density * 3. + 1e-10  
# 0.015 m, small difference in the formula from the paper.
filter_power = 2
#p_f

# Projection / shape parameters.
beta_lay = 4.
# Used in the softmax calculations.
beta_topol = 4.
# Used in the computation of \rho = material density. Ideally the variable should
# be binary. The higher the value, stronger the the binarization.

# Continuation; we do not use continuation here
beta_lay_max = beta_lay * 2**0
beta_topol_max = beta_topol * 2**0

# Augmented Lagrangian related 
# la_* are the k1, k2, k3, k4 in the paper.
# sigma_* are the T_1, T_2, T_3, T_4 in the paper.
# Notice there are 4 inequality constraints. Arbitrary initial values set here. 
# They will be updates sequentially during optimization.
la_l2_pen = 0.
la_quad_pen = 0.
la_pw_sgn_pen = 0.
la_pw_abs_pen = 0.
...
K_pw_sgn_pen_prev = 1. * 0.99  # 99% of max, much stricter
K_pw_abs_pen_prev = 0.25 * 0.99 # 99% of max, much stricter
...
#
# Physical parameters.
# Material properties of soft silicone rubber.
...
vec_f32 = lambda: ti.Vector.field(dim, dtype=ti.f32) #floating point 32
mat = lambda: ti.Matrix.field(dim, dim, dtype=real)

actuator_id = ti.field(ti.i32) #int 32
particle_type = ti.field(ti.i32)
x, v = vec(), vec()
C, F = mat(), mat()
x_viz = vec_f32()
grid_v_in, grid_m_in = vec(), scalar() #G2P
grid_v_out = vec() #P2G
loss = scalar()
...
def compute_loss(lag_coeffs: ti.types.matrix(n_penalty, 2, real)):
    ...
    dist_y = v_timesum[None][1] # uncomment for climber
...
window = ti.ui.Window("Diffmpm", (800, 800), vsync=False, show_window=False)
...
def plot_actuation(steps, actuation):
    """
    Plot the time evolution of the candidate actuation signals for each actuator.
    steps : int
        Final time step index to plot.
    actuation : ti.field (shape: (max_steps+1, n_actuators), dtype=real)
        Taichi field holding the candidate actuation signal per actuator at each
        time step.
    """
...
def plot_actuation_w_pulse(steps, actuation, pulse_w_sgn, pulse_w_abs):
    """
    Plot the time evolution of the candidate actuation signals for each actuator,
    together with the pulse actuation signals (before multiplying by act_strength) 
    for visualization.
    steps : int
        Final time step index to plot.
    actuation : ti.field (shape: (max_steps+1, n_actuators), dtype=real)
        Taichi field holding the candidate actuation signal per actuator at each
        time step.
    pulse_w_sgn : ti.field (shape: (pulse_steps, n_actuators), dtype=int)
        Taichi field holding the sign of the pulse actuation signal per actuator at each pulse time step.
    pulse_w_abs : ti.field (shape: (pulse_steps, n_actuators), dtype=real)
        Taichi field holding the absolute value of the pulse actuation signal per actuator at each pulse time step.
    """
...
def select_3x3_center_particles(scene, rect_w=0.2, rect_h=0.2, nx=3, ny=3):
    """
    Divide the design rectangle into nx×ny bins and pick 1 design particle per bin:
    choose the particle closest to the bin center using INITIAL positions scene.x.

    Returns:
      selected_p: list of particle indices (len=nx*ny)
      centers:    list of bin centers in world coords
    """
    pos = np.asarray(scene.x, dtype=float)           # (n_particles, 2)
    isd = np.asarray(scene.is_design, dtype=int)     # (n_particles,)

    # Design rectangle bounds for walker_2d: origin is the scene offset
    x0 = float(scene.offset_x)
    y0 = float(scene.offset_y)
    w  = float(rect_w)
    h  = float(rect_h)
    ...
    design_idx = np.where(isd == 1)[0]
    design_pos = pos[design_idx]
    ...
    for iy in range(ny):
        for ix in range(nx):
            ...
            mask = (
                (design_pos[:, 0] >= x_lo) & (design_pos[:, 0] < x_hi) &
                (design_pos[:, 1] >= y_lo) & (design_pos[:, 1] < y_hi)
            )
            cand_local = np.where(mask)[0]

            if cand_local.size == 0:
                # fallback: nearest design particle to bin center
                d2 = (design_pos[:, 0] - cx) ** 2 + (design_pos[:, 1] - cy) ** 2
                best_local = int(np.argmin(d2))
            else:
                cand_pos = design_pos[cand_local]
                d2 = (cand_pos[:, 0] - cx) ** 2 + (cand_pos[:, 1] - cy) ** 2
                best_local = int(cand_local[np.argmin(d2)])

            p = int(design_idx[best_local])
            selected_p.append(p)

    return selected_p, centers

def plot_config_actuator_if_solid_else_void(
    x_pos_np,               # (N,2)
    rho_np,                 # (N,) physical/material density in [0,1] (or close)
    act_w_soft_np,          # (N, n_actuators+1) last column = "no actuator"
    n_actuators=4,
    rho_thr=0.10,
    tracked_particles=None, # list of particle indices to mark
    outpath=None,
    title=None,
    marker_size=18,
    iter=None
...
"""
    ...
    assert x_pos_np.shape[1] == 2
    assert W.shape[0] == x_pos_np.shape[0]
    assert W.shape[1] >= n_actuators + 1, "act_w_soft should include +1 for 'no actuator'"

    N = x_pos_np.shape[0]

    # 1) solid/void mask from physical density
    solid = rho_np > rho_thr

    # 2) actuator assignment: 0..n_actuators-1, and n_actuators = "no actuator"
    assign = np.argmax(W[:, :n_actuators+1], axis=1)  # shape (N,)

    # 3) override void
    VOID_CLASS = n_actuators + 1
    cls = assign.copy()
    cls[~solid] = VOID_CLASS

    # class labels
    labels = [f"Actuator {k+1}" for k in range(n_actuators)] + ["No actuators", "Void"]

    # colors (edit if you want paper-like colors)
    colors = [
        "#1f77b4",  # actuator 1
        "#ff7f0e",  # actuator 2
        "#2ca02c",  # actuator 3
    ...
    bounds = np.arange(-0.5, n_actuators + 2.5, 1.0)
    ...
    sc = ax.scatter(
        x_pos_np[:, 0], x_pos_np[:, 1],
        c=cls, cmap=cmap, norm=norm,
        s=marker_size, marker="s", linewidths=0.0
    )

    # 4) overlay tracked particles
    if tracked_particles:
        tracked_particles = [p for p in tracked_particles if 0 <= p < N]
        tp = x_pos_np[tracked_particles]
        ax.scatter(tp[:, 0], tp[:, 1], s=120, c="k", marker="x", linewidths=2.0, zorder=5)

    # Legend
    handles = [Patch(facecolor=colors[i], edgecolor="none", label=labels[i]) for i in range(n_actuators + 2)]
...
def main():
    ...
    design_climber_2d(scene)
    # design_balancer_2d(scene)
    #design_cube(scene)
    scene.finalize()
    ...
    # ADDED: Choose 9 fixed design particles (3×3 bins) 
    # BEFORE the first iteration begins
    tracked_particles, bin_centers = select_3x3_center_particles(
        scene,rect_w=0.2, rect_h=0.2, nx=3, ny=3
        )
    print(f"\n[TRACKING] 3x3 bin centers and chosen particle IDs \
     (fixed, based on initial positions):")
    pos0 = np.asarray(scene.x, dtype=float)
    for idx, (p, c) in enumerate(zip(tracked_particles, bin_centers)):
        print(f"  bin {idx:02d} center={c} -> p={p}, x0={pos0[p].tolist()}")
    ...
    bin_operator = ParticleGridPlotter(
        x0= float(scene.offset_x),
        y0= float(scene.offset_y), 
        w= 0.2, h=0.2, nx=3, ny=3,
        dx = dx, n_grid = n_grid,
        bound_x_min_np= bound_x_min.to_numpy(), 
        bound_x_max_np= bound_x_max.to_numpy(), 
        bound_y_min_np= bound_y_min.to_numpy(),
        bound_y_max_np=bound_y_max.to_numpy()
    )
    actuation_operator = ParticleActuationAnalyzer(
        n_actuators=n_actuators,
        dt=dt,
        act_strength=act_strength,
        matd=matd,
        penal_act=penal_act
    )
    ...
    while not STOP_ITER:
        ...
        if CONVERGED:
            outpath = f"{folder}/config/config_iter{iter:04d}.png"
            plot_config_actuator_if_solid_else_void(
            x_pos_np=x.to_numpy()[max_steps],
            rho_np=rho.to_numpy(),
            act_w_soft_np=act_w_soft.to_numpy(),
            n_actuators=n_actuators,
            rho_thr=0.10,
            tracked_particles=[246, 259, 273, 766, 779, 793, 1326, 1339, 1353],
            outpath=outpath,
            title=f"iter={iter:04d}", iter=iter) 

        if SAVE_PNG:  # i.e. the same iterations where you save frames / gifs
            out_root = os.path.join(folder, 'iter{:04d}'.format(iter))
            bins_out = os.path.join(out_root, f"bins_iter{iter:04d}_final.png")
            bin_operator.plot(
                x_pos_np= x.to_numpy()[max_steps],
                is_design_np= is_design.to_numpy(),
                tracked_particles=tracked_particles,
                outpath=bins_out,
                title=f"Bins + particles (final config) | iter={iter:04d}"
            )
            
            for p in tracked_particles:
                out_root = os.path.join(folder, "particles")
                p_folder = os.path.join(out_root, f"{p}")
                os.makedirs(p_folder, exist_ok=True)
                t, u_hat, u_sum, target_j, act_w_soft_target, simp_act = actuation_operator.compute_curves(
                    p=p,
                    total_steps=max_steps,
                    actuation_np= actuation.to_numpy(),
                    act_w_soft_np= act_w_soft.to_numpy(),
                    is_design_np= is_design.to_numpy(),
                    design_id_np= design_id.to_numpy(),
                    rho_np= rho.to_numpy()
                )
                if u_hat is None:
                    continue
                ...
                np.savez(
                    os.path.join(p_folder, f"u_hat_iter{iter:04d}.npz"),
                    t=t,
                    u_hat=u_hat,
                    u_sum=u_sum,
                    target_j=np.array([target_j], dtype=int),
                    act_w_soft_target=np.array([act_w_soft_target], dtype=float),
                )
                actuation_operator.plot_single_iter(
                    particle_id=p,
                    iter_idx=iter,
                    t=t,
                    u_hat=u_hat,
                    u_sum=u_sum,
                    act_w_soft_target=act_w_soft_target,
                    target_j=target_j,
                    outpath=os.path.join(p_folder, f"u_hat_iter{iter:04d}.png"),
                )
                actuation_operator.plot_stacked_history(p_folder, particle_id=p)
    ...
    print(f"\nTOTAL runtime: {total_sec/60:.2f} minutes ({total_sec:.1f} s)\n")
